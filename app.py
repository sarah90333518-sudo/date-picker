...
with st.sidebar:
    st.title("🌲 약속의 숲")
    st.caption("Streamlit 버전 약속 날짜 투표기")
    ...
    if not st.session_state.polls:
        ...
        for p_id, poll in st.session_state.polls.items():
            if st.button(f"📌 {poll['title']}", key=p_id, use_container_width=True):
...
with tab_create:
    ...
    with st.form("create_poll_form"):
        ...
        # Candidate Dates Selection
        today = date.today()
        candidate_dates = st.date_input(
            "후보 날짜 선택 (범위 지정 또는 복수 선택) *",
            value=(today, today + timedelta(days=7)),
            min_value=today
        )
        
        submit_create = st.form_submit_button("🚀 약속 투표 생성하기", use_container_width=True)
        ...
        if submit_create:
            if not poll_title:
                ...
                # Format dates
                date_list = []
                if isinstance(candidate_dates, tuple):
                    if len(candidate_dates) == 2:
                        start_d, end_d = candidate_dates
                        curr = start_d
                        while curr <= end_d:
                            date_list.append(curr.strftime("%Y-%m-%d"))
                            curr += timedelta(days=1)
                    else:
                        date_list.append(candidate_dates[0].strftime("%Y-%m-%d"))
                elif isinstance(candidate_dates, date):
                    date_list.append(candidate_dates.strftime("%Y-%m-%d"))
                ...
                st.success(f"'{poll_title}' 약속 투표가 생성되었습니다! '일정 투표하기' 탭으로 이동하세요.")
                st.balloons()
...
with tab_vote:
    if not active_poll:
        st.warning("선택된 약속이 없습니다. 먼저 약속을 생성하거나 사이드바에서 선택해주세요.")
    else:
        ...
        selected_existing = st.selectbox("투표자 선택 (새 참가자 또는 기존 수정)", ["+ 새 참가자로 작성하기"] + voter_names)
        ...
        if st.button("💾 내 일정 제출하기", use_container_width=True, type="primary"):
            if not voter_name.strip():
                ...
                st.success(f"{voter_name}님의 투표가 완료되었습니다!")
                st.balloons()
